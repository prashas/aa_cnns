...
def remove_urls(raw_text, replace_with='U'):
...
def replace_tweet_usernames(raw_text, replace_with='N'):
    clean_text = raw_text
...
def replace_tweet_numbers(raw_text, replace_with='R'):
    clean_text = re.sub(r"[+-]?\d+(?:\.\d*)?(?:[eE][+-]?\d+)*", replace_with, raw_text)
    return clean_text

def replace_tweet_date_time(raw_text, replace_with='T'):
    clean_text = re.sub(r"(\d+[/-]\d+[/-]\d+)", 'D', raw_text)
