...
from load_data.ds_get_data import get_data_for_simple_baseline_authorship_gender
from load_data.get_data import Features, get_authors_train_test_koppel_vauthors, convert_data_for_nn_models, \
...
def get_data_for_nns(params):
    ...
    if dataset == "tweet":
        expt_type = "all_authors"
        rootdir = os.path.join(os.path.join(resources_rootdir, "koppel_tweets", "cleaned_all", expt_type))
        authors = get_author_names_all_authors(os.path.join(resources_rootdir, "koppel_tweets", "all_authors.txt"))
        test_fold = 0
        ...
        rootdir = os.path.join(resources_rootdir, "ds_aa")
        aa_rootdir = os.path.join(rootdir, "depression_aa_500authors_300posts_cleaned_balanced")
        authors = [x.strip() for x in futils.get_lines_in_file_small(os.path.join(rootdir, "ds_authors_500.txt"))]
        authors = authors[sample_id * authors_count:(sample_id + 1) * authors_count]
        ...
        rootdir = os.path.join(resources_rootdir, "amazon")
        aa_rootdir = os.path.join(rootdir, "amazon_cleaned_balanced")
        authors = [x.strip() for x in futils.get_lines_in_file_small(os.path.join(rootdir, "amazon_authors_balanced_500.txt"))]
        authors = authors[sample_id * authors_count:(sample_id + 1) * authors_count]
...
def train_model_with_params(params):
    ...
    models_results_opdir = "{}_{}".format(os.path.join(results_opdir, details_for_fname), time.strftime("%Y%m%d-%H%M%S"))
    ...
    if model_type == "multichannel":
        ...
        # model = get_multichannel_model(authors, X_train_char, X_train_word, char_to_idx, word_to_idx, embedding_dims, filter_tuple, activation_value, dropout, channel1_weights=char_embedding_matrix, channel2_weights=embedding_matrix)
